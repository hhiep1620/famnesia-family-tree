...
export function FamilyTreePage({ user, onSignOut }: Props) {
  ...
  return <main className="app-shell">
    <AppHeader persons={data.persons} media={data.media} workspaceId={data.workspace?.id} profiles={data.profiles} activeProfileId={data.activeProfileId} workspaces={data.workspaces} activeWorkspaceId={data.activeWorkspaceId} canEdit={canEdit} user={user} mock={data.useMockData} view={view} subject={subject} kinships={kinships} scopes={scopes} refreshing={data.loading} onProfileChange={data.setActiveProfileId} onWorkspaceChange={data.switchWorkspace} onViewChange={changeView} onSearch={openRelativeExplorer} onAdd={() => setModal({ type: 'add' })} onRefresh={() => void data.refresh()} onSignOut={onSignOut} />
