...
export function FamilyTreePage({ user, onSignOut, runtimeRepository }: Props) {
  ...
  const routeView: MainView = location.pathname.endsWith('/calendar') ? 'calendar' : location.pathname.endsWith('/analytics') ? 'analytics' : location.pathname.endsWith('/members') || location.pathname.endsWith('/settings') ? 'data' : 'tree'
  const [view, setView] = useState<MainView>(routeView)
  const [selectedId, setSelectedId] = useState<string>()
  const [searchParams, setSearchParams] = useSearchParams()
  const treeFilter = (['all', 'birthday', 'death_anniversary'] as const).includes(searchParams.get('filter') as TreeFilter) ? searchParams.get('filter') as TreeFilter : 'all'
  ...
  const goToView = (nextView: MainView) => {
    setView(nextView); setSelectedId(undefined); setExplorerId(undefined)
    if (runtimeRepository && data.activeWorkspaceId) navigate(`/workspaces/${data.activeWorkspaceId}/${nextView === 'data' ? 'settings' : nextView}`)
  }
  const changeView = (nextView: MainView) => { goToView(nextView) }
  const changeTreeFilter = (filter: TreeFilter) => {
    const next = new URLSearchParams(searchParams)
    if (filter === 'all') next.delete('filter')
    else next.set('filter', filter)
    setSearchParams(next)
  }
  const openCalendarPerson = (id: string) => { goToView('calendar'); setSelectedId(id) }
  const openRelativeExplorer = (id: string) => { goToView('tree'); setExplorerId(id) }
  const openFamilyBranch = (id: string) => { goToView('tree'); setViewSubjectStack((current) => current.at(-1) === id ? current : [...current, id]) }
  const goBackSubject = () => { setSelectedId(undefined); setViewSubjectStack((current) => current.length > 1 ? current.slice(0, -1) : current) }
  const openImport = () => { setOpenImportOnMount(true); goToView('data') }
  ...
  const requestWorkspaceChange = (id: string) => guardLeave('Chuyển sang workspace khác?', async () => { await data.switchWorkspace(id); navigate(`/workspaces/${encodeURIComponent(id)}/tree`) })
  ...
  return <main className="app-shell">
          ...
          : explorerId ? <RelativeExplorer targetId={explorerId} graph={graph} media={data.media} workspaceId={data.workspace?.id} onClose={() => setExplorerId(undefined)} onOpenPerson={setExplorerId} onViewFullTree={openFamilyBranch} />
            ...
            : data.profiles.length === 0 ? canEdit ? <StartFamilyTree onCreate={() => setProfileModal('create')} onImport={canReplaceData ? openImport : undefined} onDownloadTemplate={downloadFamilyDataTemplate} sharedWorkspaceMode={data.useMockData ? undefined : data.workspace?.rootFolderUrl ? 'drive' : 'invite'} onConnectSharedWorkspace={data.workspace?.rootFolderUrl ? data.connectSharedWorkspace : undefined} /> : <div className="center-state empty-state"><h2>Workspace chưa có dữ liệu</h2><p>Bạn có quyền xem. Hãy nhờ owner hoặc editor bổ sung gia đình.</p></div>
              : data.persons.length ? view === 'tree' ? <>
                <div className="tree-filter-bar"><span>Đánh dấu trong 30 ngày:</span><button className={treeFilter === 'all' ? 'active' : ''} onClick={() => changeTreeFilter('all')}>Tất cả</button><button className={treeFilter === 'birthday' ? 'active' : ''} onClick={() => changeTreeFilter('birthday')}><CakeSlice size={14} /> Sinh nhật</button><button className={treeFilter === 'death_anniversary' ? 'active' : ''} onClick={() => changeTreeFilter('death_anniversary')}><Flower2 size={14} /> Ngày giỗ</button></div>
              ...
              </> : view === 'calendar' ? <FamilyCalendar persons={data.persons} onOpenPerson={setSelectedId} onViewTree={openFamilyBranch} /> : <FamilyAnalytics data={activeFamilyData} subject={subject} onOpenCalendar={() => goToView('calendar')} />
    ...
    {profileModal && <CreateProfileModal mode={profileModal} profile={profileModal === 'edit' ? data.activeProfile : undefined} suggestedSurnames={suggestedSurnames} busy={data.busy} onClose={() => setProfileModal(undefined)} onSubmit={async (name, description, lineageSurname) => { if (profileModal === 'edit' && data.activeProfile) await data.updateProfile(data.activeProfile.id, name, description, lineageSurname); else { await data.createProfile(name, description, lineageSurname); goToView('tree') } }} />}
