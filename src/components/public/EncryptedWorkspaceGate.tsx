...
export function EncryptedWorkspaceGate({ onSignOut }: { onSignOut: () => void }) {
  ...
  return <main className="public-homepage"><header className="public-header"><BrandLogo /><nav aria-label="Tài khoản"><button className="text-action" disabled={Boolean(busy)} onClick={() => void refresh()}><RefreshCw size={15} /> Làm mới</button><button className="text-action" onClick={onSignOut}><LogOut size={15} /> Đăng xuất</button></nav></header><section className="public-hero"><div className="hero-copy"><span className="eyebrow">Encrypted Preview gate</span><h1>Workspace đã sẵn sàng; dữ liệu gia đình đang được khóa.</h1><p>Đường đọc/ghi family, media, backup và Draft plaintext đã bị chặn ở server. {joinWorkflowAvailable ? 'Database Preview đã bật create/join/owner approval.' : 'Database hiện tại chưa có crypto/join migration nên create/join được khóa rõ ràng.'}</p>{error && <p className="form-error">{error}</p>}<div className="join-box"><label htmlFor="workspace-name">Tạo workspace mới</label><div><input id="workspace-name" value={name} maxLength={120} disabled={!joinWorkflowAvailable} onChange={(event) => setName(event.target.value)} /><button className="secondary-button" disabled={!joinWorkflowAvailable || Boolean(busy) || !name.trim()} onClick={() => void createWorkspace()}><Plus size={15} /> Tạo</button></div>{!joinWorkflowAvailable && <small>Chưa bật trên database của Preview này.</small>}</div></div><div className="lineage-hero" aria-label="Trạng thái mã hóa"><KeyRound size={42} /><strong>Fail closed</strong><span>Không fallback về plaintext</span></div></section><section className="public-values"><span className="eyebrow">Workspace của bạn</span><div className="public-value-grid">{workspaces.length === 0 && !busy ? <article><h2>Chưa có workspace</h2><p>Tạo workspace hoặc mở link join khi database Preview đã bật migration.</p></article> : workspaces.map((workspace) => <article key={workspace.id}><h2>{workspace.name}</h2><p>Vai trò: {workspace.role}</p>{joinWorkflowAvailable && workspace.joinCode && <p><strong>Mã join: {workspace.joinCode}</strong> <button className="icon-button" aria-label="Sao chép mã join" onClick={() => void navigator.clipboard.writeText(workspace.joinCode!)}><Copy size={14} /></button></p>}{joinWorkflowAvailable && workspace.ownedByMe && (requests[workspace.id] ?? []).map((request) => <div className="join-box" key={request.request_id}><strong>{request.requester_name || request.requester_email}</strong><small>{request.requester_email} · xin quyền {request.requested_role}</small><div><button className="primary-button" disabled={Boolean(busy)} onClick={() => void resolve(workspace.id, request.request_id, true, request.requested_role)}><Check size={14} /> Duyệt</button><button className="secondary-button" disabled={Boolean(busy)} onClick={() => void resolve(workspace.id, request.request_id, false, request.requested_role)}><X size={14} /> Từ chối</button></div></div>)}</article>)}</div></section></main>
}
