...
export function DataManagement(props: Props) {
  ...
  return <div className="data-page">
    ...
    <section className="backup-section"><div className="backup-heading"><div><span className="section-label">An toàn dữ liệu</span><h3>Bản sao lưu</h3><p>Import và khôi phục luôn tạo backup tự động. Khôi phục chỉ dành cho owner.</p></div>{(props.mock || props.workspace?.canCommitDirectly) && <button className="secondary-button" onClick={() => void createBackup()} disabled={Boolean(props.busy)}><Archive size={16} /> Backup ngay</button>}</div>
      {backupError && <p className="form-error">{backupError}</p>}
      <div className="backup-list">{backups.length ? backups.map((backup) => <div className="backup-row" key={backup.id}><Archive size={16} /><div><strong>{backup.name}</strong><span>{formatBackupTime(backup.createdTime ?? backup.modifiedTime)}{backup.reason ? ` · ${backup.reason}` : ''}</span></div>{(props.mock || props.workspace?.canCommitDirectly) && <button className="secondary-button" onClick={() => void restore(backup)}><RotateCcw size={14} /> Khôi phục</button>}</div>) : <div className="backup-empty">Chưa có bản sao lưu nào.</div>}</div>
    </section>
