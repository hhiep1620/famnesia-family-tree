...
export function ActivityTimeline({ events }: { events: ActivityEvent[] }) {
  ...
  return <section className="activity-timeline"><div className="activity-filters"><button className={filter === 'all' ? 'active' : ''} onClick={() => setFilter('all')}>Tất cả</button><button className={filter === 'person' ? 'active' : ''} onClick={() => setFilter('person')}>Thành viên</button><button className={filter === 'relationship' ? 'active' : ''} onClick={() => setFilter('relationship')}>Quan hệ</button><button className={filter === 'dataset' ? 'active' : ''} onClick={() => setFilter('dataset')}>Import / backup</button></div>{filtered.length ? <div>{filtered.map((event) => <article key={event.id}><span>{icon(event.action)}</span><div><strong>{event.summary}</strong><small>{event.actorName || event.actorEmail} · {event.actorEmail}</small></div><time><Clock3 size={12} />{new Intl.DateTimeFormat('vi-VN', { dateStyle: 'short', timeStyle: 'short' }).format(new Date(event.timestamp))}</time></article>)}</div> : <div className="quality-empty"><Clock3 /><h3>Chưa có lịch sử hoạt động</h3><p>Các thay đổi mới trên Google Drive sẽ được ghi theo tháng trong thư mục activity/.</p></div>}</section>
}
