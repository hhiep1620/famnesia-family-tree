...
export function PersonDetails(props: Props) {
  ...
  const { url } = useDriveImage(workspaceId, mediaReferenceId(primaryMedia))
  ...
  return <aside className="person-details" aria-label={`Chi tiết ${person.name}`}>
    ...
    {!props.readOnly && <div className="danger-zone"><button className="danger-button" disabled={props.busy} onClick={() => { if (window.confirm(`Đánh dấu xóa ${person.name}, ${references.length} quan hệ và ${personMedia.length} ảnh trong Draft? Bạn có thể hoàn tác trước khi Lưu tất cả.`)) void props.onDeletePerson() }}><Trash2 size={15} /> Đánh dấu xóa</button>{references.length > 0 && <p>Khi Lưu tất cả, thao tác này sẽ tự xóa {parentCount} quan hệ cha/mẹ, {spouseCount} quan hệ bạn đời và {childCount} quan hệ với con.</p>}</div>}
  </aside>
}
