...
import { differenceInCalendarDays, parseIsoDate, todayInFamilyTimezone } from '../../calendar/dateUtils'
import { getFamilyEventsForMonth, getFamilyEventsForYear, getReminderLabel, getUpcomingFamilyEvents } from '../../calendar/familyCalendar'
...
function getRangeEvents(persons: Person[], range: Range, type: FamilyEventType | 'all', today: Date): FamilyEvent[] {
  if (range === 7 || range === 30) return getUpcomingFamilyEvents(persons, range, type, today)
  const events = getFamilyEventsForYear(persons, today.getFullYear()).filter((event) => type === 'all' || event.type === type)
  if (range === 'year') return events.filter((event) => differenceInCalendarDays(parseIsoDate(event.date)!, today) >= 0)
  return events.filter((event) => parseIsoDate(event.date)?.getMonth() === today.getMonth() && differenceInCalendarDays(parseIsoDate(event.date)!, today) >= 0)
}
