...
export function useFamilyData(userId = 'mock-user') {
  ...
  const addMember = useCallback(async (email: string, role: 'editor' | 'viewer'): Promise<WorkspaceInvitationResult | void> => { if (!repository.current) throw new Error('Workspace chưa sẵn sàng.'); const invitation = await repository.current.addMember(email, role); await refreshMembers(); return invitation }, [refreshMembers])
